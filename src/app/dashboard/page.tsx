...
import { Landmark, LayoutGrid, Users, LogOut } from "lucide-react";
import CompanySelector from "@/components/company-selector";

const BRAND = "#1B3A6B";

const TILES = [
  { title: "Finance",     description: "Accounts, ledgers & reconciliations", href: "/dashboard/finance",     Icon: Landmark   },
  { title: "Operational", description: "Sales, purchase & inventory",          href: "/dashboard/operational", Icon: LayoutGrid },
  { title: "HR",          description: "Employees & payslips",                 href: "/dashboard/hr",          Icon: Users      },
];
  ...
  return (
    <>
      ...
      <div className="animated-bg min-h-screen relative overflow-hidden">
        ...
        {/* Title */}
        <section className="relative z-10 px-6 md:px-10 pt-4 pb-10 text-center">
          <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Finance Manager</h1>
          <div className="mx-auto mt-3 h-[3px] w-16 rounded-full bg-white" />
        </section>

        {/* Tiles */}
        <main className="relative z-10 px-6 md:px-12 lg:px-16 pb-20 pt-2">
          <div className="mx-auto max-w-7xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {TILES.map(({ title, description, href, Icon }, i) => (
              <Link
                key={title}
                href={href}
                ...
                style={{
                  ...
                  boxShadow: "0 20px 50px -10px rgba(0,0,0,0.35),0 8px 20px -5px rgba(0,0,0,0.18),0 2px 6px rgba(0,0,0,0.08)",
                }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 30px 70px -10px rgba(15,37,71,0.45),0 14px 30px -8px rgba(27,58,107,0.3),0 4px 10px rgba(0,0,0,0.1)"; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "0 20px 50px -10px rgba(0,0,0,0.35),0 8px 20px -5px rgba(0,0,0,0.18),0 2px 6px rgba(0,0,0,0.08)"; }}
              >
