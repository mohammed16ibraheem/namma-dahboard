...
const HIST_KEY = "account_expenses_history";
...
export default function CurrentExpensesPage() {
  ...
  function handleSubmit() {
    ...
    if (filled.length === 0) { setError("Please add at least one expense entry."); return; }
    const incomplete = filled.find(r => !r.expenseName.trim() || num(r.amount) <= 0);
    if (incomplete) { setError("Each row needs an Expense Name and an Amount greater than 0."); return; }
    ...
    setError("");
    setSuccessMsg(`Expenses saved — SAR ${money(total)}`);
  ...
  function exportToExcel() {
    ...
    const sheet1Data: (string | number | null)[][] = [
      ["NAMMA DASHBOARD — CURRENT EXPENSES REPORT", null, null, null, null, null, null],
      ...
      [],
      ["#", "Company Name", "Expense Name", "Month", "Amount (SAR)", "Saved Date", "Saved Time"],
      ...allRows,
      [],
    ...
    applyStyles(ws1, { metaEnd: 4, headerRow: 6, dataStart: 7, dataEnd: 6 + allRows.length, totalRow: 8 + allRows.length, amountCols: [4], accent: "D97706", colCount: 7 });
    XLSX.utils.book_append_sheet(wb, ws1, "Expenses Report");

    const expenseMap: Record<string, { total: number; count: number }> = {};
    histData.forEach(entry => {
      entry.rows.forEach(row => {
        if (!expenseMap[row.expenseName]) expenseMap[row.expenseName] = { total: 0, count: 0 };
        expenseMap[row.expenseName].total += num(row.amount);
        expenseMap[row.expenseName].count += 1;
      });
    });
    const expEntries       = Object.entries(expenseMap).sort((a, b) => b[1].total - a[1].total);
    const expGrandTotal    = expEntries.reduce((s, [, v]) => s + v.total, 0);

    const sheet2Data: (string | number | null)[][] = [
      ["EXPENSE CATEGORY SUMMARY", null, null, null],
      [`Generated: ${new Date().toLocaleDateString("en-GB")}`, null, null, null],
      [],
      ["Expense Name", "No. of Entries", "Total (SAR)", "Share (%)"],
      ...expEntries.map(([name, v]) => [name, v.count, v.total, expGrandTotal > 0 ? parseFloat(((v.total / expGrandTotal) * 100).toFixed(2)) : 0]),
      [],
      ["TOTAL", expEntries.reduce((s, [, v]) => s + v.count, 0), expGrandTotal, 100],
    ];
    const ws2 = XLSX.utils.aoa_to_sheet(sheet2Data);
    ws2["!cols"] = [{ wch: 34 }, { wch: 16 }, { wch: 18 }, { wch: 12 }];
    applyStyles(ws2, { metaEnd: 1, headerRow: 3, dataStart: 4, dataEnd: 3 + expEntries.length, totalRow: 5 + expEntries.length, amountCols: [2], accent: "D97706", colCount: 4 });
    XLSX.utils.book_append_sheet(wb, ws2, "Expense Summary");
    ...
    const sheet3Data: (string | number | null)[][] = [
      ["MONTHLY EXPENSES SUMMARY — CHART DATA", null],
      ["Tip: Select Month & Total columns → Insert → Chart", null],
      [],
      ["Month", "Total Expenses (SAR)"],
    ...
    applyStyles(ws3, { metaEnd: 1, headerRow: 3, dataStart: 4, dataEnd: 3 + sortedMonths.length, totalRow: 5 + sortedMonths.length, amountCols: [1], accent: "D97706", colCount: 2 });
    ...
    const sheet4Data: (string | number | null)[][] = [
      ["COMPANY EXPENSES SUMMARY", null, null, null],
      [`Generated: ${new Date().toLocaleDateString("en-GB")}`, null, null, null],
      [],
    ...
    applyStyles(ws4, { metaEnd: 1, headerRow: 3, dataStart: 4, dataEnd: 3 + companyEntries.length, totalRow: 5 + companyEntries.length, amountCols: [2], accent: "D97706", colCount: 4 });
  ...
  const histNameTotals = useMemo(() => {
    ...
    histData.forEach(e => e.rows.forEach(r => { map[r.expenseName] = (map[r.expenseName] ?? 0) + num(r.amount); }));
                  ...
                  {rows.map((row, idx) => (
                    <tr key={row.id} className={`border-b border-gray-100 ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/40"}`}>
                      ...
                      <td className="px-3 py-2.5">
                        <input type="text" placeholder="e.g. Office Rent"
              ...
              <button onClick={() => setHistOpen(o => !o)} className="flex items-center gap-2.5 group">
                ...
                <div className="text-left">
                  <h2 className="text-lg font-bold text-gray-800 group-hover:text-[#1B3A6B] transition-colors">
                    Submission History
                    {histData.length > 0 && (
                      <span className="ml-2 text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#FFFBEB", color: ACCENT }}>
                        {histData.length} saved
                      </span>
                    )}
                  </h2>
                  <p className="text-xs text-gray-400">All submitted expense entries across companies</p>
                </div>
              ...
              <>
                {histData.length === 0 ? (
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-16 flex flex-col items-center gap-3 text-center">
                    ...
                    <p className="text-xs text-gray-300">Save your first expense entry above to see it here</p>
                  </div>
                ) : (
                  <>
                    ...
                    {histNameTotals.entries.length > 0 && (
                      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                        <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-4">Expenses by Category</h3>
                        ...
                        <table className="w-full text-sm min-w-[520px]">
                          <thead>
                            <tr className="text-[10px] uppercase tracking-wider text-gray-400 border-b border-gray-100">
                              ...
                              <th className="px-4 py-2 text-left">Expense Name</th>
                          ...
                          <tbody>
                            {entry.rows.map((row, ri) => (
                              <tr key={row.id ?? ri} className={`border-b border-gray-100 ${ri % 2 === 0 ? "bg-white" : "bg-gray-50/40"}`}>
                                ...
                                <td className="px-4 py-2.5 text-sm font-medium text-gray-700">{row.expenseName}</td>
