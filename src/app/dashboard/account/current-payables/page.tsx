...
let nextId = 1;
const makeRow = (): CustomerRow => ({ id: nextId++, customerName: "", month: CURRENT_MONTH, amount: "" });

const num   = (v: string) => { const n = parseFloat(v); return Number.isFinite(n) ? n : 0; };
const money = (n: number) => n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const coName = (id: string) => COMPANIES.find(c => c.id === id)?.name ?? id;

const HIST_KEY = "account_payables_history";

export default function CurrentPayablesPage() {
  const router = useRouter();
  const [rows, setRows]           = useState<CustomerRow[]>([makeRow(), makeRow()]);
  ...
  function updateRow(id: number, field: keyof Omit<CustomerRow, "id">, value: string) {
  ...
  function handleSubmit() {
    const filled = rows.filter(r => r.customerName.trim() || num(r.amount) > 0);
    if (filled.length === 0) { setError("Please add at least one customer entry."); return; }
    const incomplete = filled.find(r => !r.customerName.trim() || num(r.amount) <= 0);
    if (incomplete) { setError("Each row needs a Customer Name and an Amount greater than 0."); return; }

    const companyId   = localStorage.getItem("active_company") ?? "diamond-star";
    const companyName = coName(companyId);

    const existing = JSON.parse(localStorage.getItem("account_payables") ?? "{}");
    existing[companyId] = { rows: filled, total };
    localStorage.setItem("account_payables", JSON.stringify(existing));
    ...
    setError("");
    setSuccessMsg(`Payables saved — SAR ${money(total)}`);
  ...
  function exportToExcel() {
    ...
    const sheet1Data: (string | number | null)[][] = [
      ["NAMMA DASHBOARD — CURRENT PAYABLES REPORT", null, null, null, null, null, null],
      ...
      [],
      ["#", "Company Name", "Customer Name", "Month", "Amount (SAR)", "Saved Date", "Saved Time"],
      ...allRows,
      [],
    ...
    applyStyles(ws1, { metaEnd: 4, headerRow: 6, dataStart: 7, dataEnd: 6 + allRows.length, totalRow: 8 + allRows.length, amountCols: [4], accent: "DC2626", colCount: 7 });
    XLSX.utils.book_append_sheet(wb, ws1, "Payables Report");

    // Sheet 2: Customer Summary
    const custMap: Record<string, { total: number; count: number }> = {};
    histData.forEach(entry => {
      entry.rows.forEach(row => {
        if (!custMap[row.customerName]) custMap[row.customerName] = { total: 0, count: 0 };
        custMap[row.customerName].total += num(row.amount);
        custMap[row.customerName].count += 1;
      });
    });
    const custEntries  = Object.entries(custMap).sort((a, b) => b[1].total - a[1].total);
    const custGrandTotal = custEntries.reduce((s, [, v]) => s + v.total, 0);

    const sheet2Data: (string | number | null)[][] = [
      ["CUSTOMER SUMMARY", null, null, null],
      [`Generated: ${new Date().toLocaleDateString("en-GB")}`, null, null, null],
      [],
      ["Customer Name", "No. of Entries", "Total (SAR)", "Share (%)"],
      ...custEntries.map(([name, v]) => [name, v.count, v.total, custGrandTotal > 0 ? parseFloat(((v.total / custGrandTotal) * 100).toFixed(2)) : 0]),
      [],
      ["TOTAL", custEntries.reduce((s, [, v]) => s + v.count, 0), custGrandTotal, 100],
    ];
    const ws2 = XLSX.utils.aoa_to_sheet(sheet2Data);
    ws2["!cols"] = [{ wch: 34 }, { wch: 16 }, { wch: 18 }, { wch: 12 }];
    applyStyles(ws2, { metaEnd: 1, headerRow: 3, dataStart: 4, dataEnd: 3 + custEntries.length, totalRow: 5 + custEntries.length, amountCols: [2], accent: "DC2626", colCount: 4 });
    ...
    const sheet3Data: (string | number | null)[][] = [
      ["MONTHLY PAYABLES SUMMARY — CHART DATA", null],
      ["Tip: Select Month & Total columns → Insert → Chart", null],
      [],
      ["Month", "Total Payables (SAR)"],
    ...
    applyStyles(ws3, { metaEnd: 1, headerRow: 3, dataStart: 4, dataEnd: 3 + sortedMonths.length, totalRow: 5 + sortedMonths.length, amountCols: [1], accent: "DC2626", colCount: 2 });
    ...
    const sheet4Data: (string | number | null)[][] = [
      ["COMPANY PAYABLES SUMMARY", null, null, null],
      [`Generated: ${new Date().toLocaleDateString("en-GB")}`, null, null, null],
      [],
    ...
    applyStyles(ws4, { metaEnd: 1, headerRow: 3, dataStart: 4, dataEnd: 3 + companyEntries.length, totalRow: 5 + companyEntries.length, amountCols: [2], accent: "DC2626", colCount: 4 });
    XLSX.utils.book_append_sheet(wb, ws4, "Company Summary");

    XLSX.writeFile(wb, `Payables_Report_${new Date().toISOString().split("T")[0]}.xlsx`);
  }

  const histNameTotals = useMemo(() => {
    const map: Record<string, number> = {};
    histData.forEach(e => e.rows.forEach(r => { map[r.customerName] = (map[r.customerName] ?? 0) + num(r.amount); }));
      ...
      <main className="flex-1 w-full px-4 md:px-8 py-8">
        <div className="mx-auto max-w-4xl">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-gray-400 mb-4">
            ...
            <ChevronRight size={12} />
            <span className="text-gray-600 font-medium">Current Payables</span>
          </nav>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Current Payables</h1>
              <p className="text-sm text-gray-500">Add customer entries and calculate total payment</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-5">
            <div className="px-5 py-3 border-b border-gray-100 bg-gray-50/60 flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-gray-500">Customer Entries</span>
              <span className="text-xs text-gray-400">
                {rows.filter(r => r.customerName.trim() || num(r.amount) > 0).length} entr{rows.filter(r => r.customerName.trim() || num(r.amount) > 0).length === 1 ? "y" : "ies"}
              </span>
            </div>
              ...
              <button onClick={() => setHistOpen(o => !o)} className="flex items-center gap-2.5 group">
                ...
                <div className="text-left">
                  <h2 className="text-lg font-bold text-gray-800 group-hover:text-[#1B3A6B] transition-colors">
                    Submission History
                    {histData.length > 0 && (
                      <span className="ml-2 text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#FEF2F2", color: ACCENT }}>
                        {histData.length} saved
                      </span>
                    )}
                  </h2>
                  <p className="text-xs text-gray-400">All submitted payable entries across companies</p>
                </div>
              ...
              <>
                {histData.length === 0 ? (
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-16 flex flex-col items-center gap-3 text-center">
                    ...
                    <p className="text-xs text-gray-300">Save your first payables entry above to see it here</p>
                  </div>
                ) : (
                  <>
                    ...
                    {histNameTotals.entries.length > 0 && (
                      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                        <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-4">Payables by Customer</h3>
                          ...
                          <tbody>
                            {entry.rows.map((row, ri) => (
                              <tr key={row.id ?? ri} className={`border-b border-gray-100 ${ri % 2 === 0 ? "bg-white" : "bg-gray-50/40"}`}>
                                ...
                                <td className="px-4 py-2.5 text-sm font-medium text-gray-700">{row.customerName}</td>
