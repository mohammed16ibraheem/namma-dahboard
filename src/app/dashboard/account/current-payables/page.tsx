...
// Generate months: 2 years back → 2 years forward (month + year only, no time)
const MONTHS: string[] = (() => {
  const list: string[] = [];
  const now = new Date();
...
export default function CurrentPayablesPage() {
  ...
  function handleSubmit() {
    ...
    if (filled.length === 0) {
      setError("Please add at least one customer entry.");
      return;
    }
    const incomplete = filled.find((r) => !r.customerName.trim() || num(r.amount) <= 0);
    if (incomplete) {
      setError("Each row needs a Customer Name and an Amount greater than 0.");
      return;
    }
      ...
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#F4F6FA" }}>
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="flex flex-col items-center gap-4 text-center max-w-md">
            ...
            <h2 className="text-2xl font-bold text-gray-800">Payables Saved</h2>
